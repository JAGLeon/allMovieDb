...
const moviesController = {
  ...
  detail: (req, res) => {
      fetch(`${BASE_URL}/movies/${req.params.id}`)
  ...
  detailOmbd: (req, res) => {
    const queryTitulo = req.body.searchMovie;

    db.Movie.findOne({
        where: {title: {[Op.like]: '%' + queryTitulo  + '%'}},
        include : ['genre']   
    })
    .then( data => {
        if (data == null) {
            fetch(`http://www.omdbapi.com/?apikey=acd18b2&t=${queryTitulo}`)
            .then(res => res.json())
